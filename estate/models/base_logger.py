...
class Base(models.AbstractModel):
    ...
    def create(self, vals_list):
        ...
        if self._should_log():
            ...
            for record in records:
                # Filter for customer invoices if account.move
                if self._name == 'account.move' and record.move_type not in ['out_invoice', 'out_refund']:
                    continue
                try:
    ...
    def write(self, vals):
        ...
        # Filter recordset for account.move to only log customer invoices/refunds
        records_to_log = self
        if self._name == 'account.move':
            records_to_log = self.filtered(lambda r: r.move_type in ['out_invoice', 'out_refund'])
    ...
    def read(self, fields=None, load='_classic_read'):
        ...
        # Log read operations for estate and customer invoices when viewed individually (len == 1)
        if len(self) == 1 and not self.env.context.get('skip_read_log'):
            if self._name in ['estate', 'estate.type', 'account.move']:
                if self._should_log():
                    try:
                        from odoo.http import request
                        if request and request.httprequest:
                            # Use skip_read_log context key to prevent recursion when reading fields
                            self_sudo = self.sudo().with_context(skip_read_log=True)
                            
                            # Filter customer invoices/refunds
                            if self._name == 'account.move' and self_sudo.move_type not in ['out_invoice', 'out_refund']:
                                return res
