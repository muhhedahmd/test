...
import { selectOperators } from "@advanced_web_domain_widget_original_18/core/domain_selector/domain_selector_operators";
import { DateSelectionBits } from "@advanced_web_domain_widget_original_18/dateSelectionBits/dateSelectionBits";
import {
  deserializeDate,
  deserializeDateTime,
  serializeDate,
...
import {
  Expression,
  formatValue,
} from "@advanced_web_domain_widget_original_18/core/domain_tree";
import {
...
export class Editor extends Component {
  ...
  static template = "advanced_web_domain_widget.DomainSelector.Editor";
...
export class PathEditor extends Editor {
  ...
  static template = "advanced_web_domain_widget.DomainSelector.PathEditor";
...
class Input extends Component {
  ...
  static template = "advanced_web_domain_widget.DomainSelector.Input";
}

class Select extends Component {
  static props = ["value", "update", "options"];
  static template = "advanced_web_domain_widget.DomainSelector.Select";
...
class TagInput extends Component {
  ...
  static props = ["value", "update"];
  static template = "advanced_web_domain_widget.DomainSelector.TagInput";
  get value() {
...
class Range extends Component {
  ...
  static template = "advanced_web_domain_widget.DomainSelector.Range";
...
const DATETIME_EDITOR_DATE_FILTER = makeEditor(DateSelectionBits, {
  props: ({ value, update, fieldDef }) => ({
    value,
    update,
    editorInfo: getEditorInfo(fieldDef),
    ...
    default: makeEditor(DateTimeInput, {
      props: ({ value, update, fieldDef }) => ({
        ...
        onApply: (value) =>
          update(
            value
              ? genericSerializeDate(fieldDef.type, value)
              : DateTime.local()
          ),
      }),
    }),
    date_filter: DATETIME_EDITOR_DATE_FILTER,
...
const RELATIONAL_EDITOR_EQUALITY = makeEditor(
  DomainSelectorSingleAutocomplete,
  {
    props: ({ value, update, fieldDef }) => {
      return {
...
RELATIONAL_EDITOR_EQUALITY.shouldResetValue = (value) =>
  value !== false || !isId(value);
...
export function getFieldInfo(fieldDef) {
  const { is_property, type, name } = fieldDef || {};
  if(name === "id")
...
export function getOperatorsInfo(fieldDef, resModel="") {
  if(!fieldDef.relation && fieldDef.name == "id")
    fieldDef.relation = resModel;
  const descr = getFieldInfo(fieldDef);
