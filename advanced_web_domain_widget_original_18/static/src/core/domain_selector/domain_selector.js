...
import {
  ...
  useLoadDisplayNames,
} from "@advanced_web_domain_widget_original_18/core/domain_selector/utils";
import { Component, onWillStart, onWillUpdateProps } from "@odoo/owl";
import { Domain } from "@advanced_web_domain_widget_original_18/core/domain";
...
import {
  getOperatorInfo,
  toOperator,
} from "@advanced_web_domain_widget_original_18/core/domain_selector/domain_selector_operators";
import {
  Editor,
  PathEditor,
  getDefaultOperator,
  ...
  getOperatorsInfo,
  getEditorInfo,
} from "@advanced_web_domain_widget_original_18/core/domain_selector/domain_selector_fields";
import { ModelFieldSelector } from "@web/core/model_field_selector/model_field_selector";
import { formatValue } from "@advanced_web_domain_widget_original_18/core/domain_tree";
...
export class DomainSelector extends Component {
  static template = "advanced_web_domain_widget._DomainSelector";
  static components = {
    Dropdown,
    DropdownItem,
    ModelFieldSelector,
  ...
  async onPropsUpdated(p) {
    ...
    if (!defaultLeafDomain) {
      defaultLeafDomain = new Domain(
        await this.getDefaultLeafDomain(p.resModel)
      );
    }
    ...
    if (p.readonly) {
      const idsByModel = extractIdsFromDomain(
        domain,
    ...
    this.tree = buildDomainSelectorTree(
      domain,
      this.getFieldDef.bind(this),
      options
    );
  ...
  notifyChanges() {
    ...
    const archiveDomain = this.includeArchived
  ...
  insertLeaf(parent, node) {
    const newNode =
      node.type === "condition" ? cloneTree(node) : this.createNewLeaf();
  ...
  getDescription(node) {
    ...
    return leafToString(
      ...
      operatorInfo,
      value,
      this.displayNames[fieldDef?.relation]
    );
  }
  ...
  updatePath(node, path, { fieldDef } = {}) {
    if (!path) {
      ...
      if(!fieldDef.relation)
        fieldDef.relation = this.props.resModel || "";
  ...
  updateLeafOperator(node, operatorKey) {
    ...
    if (
      !editorInfo.isSupported(node.value) ||
      editorInfo.shouldResetValue?.(node.value)
    ) {
  ...
  getOperatorsInfo(node) {
    ...
    const operatorsInfo = getOperatorsInfo(fieldDef, this.props.resModel);
    if (
      !operatorsInfo.some(
        (op) => op.operator === node.operator && op.negate === node.negate
      )
    ) {
